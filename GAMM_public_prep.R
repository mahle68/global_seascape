...
#make a single masking layer

world_lands <- st_read("/home/enourani/ownCloud/Work/GIS_files/continent_shapefile/continent.shp")

world_waters <- st_read("/home/enourani/ownCloud/Work/GIS_files/World_Seas_IHO_v3/World_Seas_IHO_v3.shp")
...
water_to_remove <- waters %>% 
  dplyr::filter(NAME %in% c("Andaman or Burma Sea", "Malacca Strait", "Persian Gulf","Red Sea")) %>% 
  st_buffer(0.1) %>% 
  st_union(np_ocean) %>% 
  st_union()


masking_layer <- region %>% 
  st_union(water_to_remove) %>% 
  st_union()

save(masking_layer, file = "masking_layer.RData")  
...
#--- STEP 3:  create a plotting function for effect plots (subplots in Fig 2) ----
