...
#version 2: using package sf and map viewer as much as possible.
...
#open annotated file
files<-list.files("movebank_annotation",pattern = ".csv",full.names = T)

ann_ls<-lapply(files,function(x){
  df<-read.csv(x,stringsAsFactors=F)
  df$delta_T<-df$ECMWF.Interim.Full.Daily.SFC.Sea.Surface.Temperature-df$ECMWF.Interim.Full.Daily.SFC.Temperature..2.m.above.Ground.#calculate delta T
  df
})


save(ann_ls,file="R_files/GFB_HB_temp_sp_filtered_15km_ann.RData") #save
