# --------EMPIRICAL STUDY------- peregrine falcon in the Eurasian-african flyway #### 
...
land_15km <- st_read("/home/enourani/ownCloud/Work/GIS_files/ne_10m_land/ne_10m_land.shp") %>% 
  st_crop(y = c(xmin = -180, xmax = 180, ymin = 0, ymax = 61)) %>% 
  st_transform(meters_proj) %>% 
  st_buffer(dist = units::set_units(15000, 'm')) %>% 
  st_transform(wgs) %>% 
  st_union()

save(land_15km,file = "R_files/land_15km.RData")
...
PF <- read.csv("data/LifeTrack Peregrine falcon.csv", stringsAsFactors = F) %>% 
  ...
  mutate(month = month(date_time),
         ...
         zone = ifelse(location.lat <= 30, "tradewind",
                              ifelse(between(location.lat, 30,60), "temperate",
                                     "other"))) %>%  #other would be the arctic zone in this case
  filter(zone != "other")
...
save(PF_sp,file = "R_files/PF_GPS_pre_filter.RData") #exclusing the arctic data

load("R_files/land_15km.RData") #named land_eur_15km
load("R_files/PF_GPS_pre_filter.RData")

sea_data <- st_difference(PF_sp,land_15km)
save(sea_data, file = "R_files/PF_GPS_sea.RData")

#### ---STEP 3: temporal filter for migration season #####
#assume migration is: autumn: Sep-Oct; no spring data...based on amount of data over water
sea_data_migr <-sea_data %>% 
  mutate(season = ifelse (month %in% c(9,10), "autumn",
                                 "other")) %>% 
  filter(season != "other")

save(sea_data_migr, file = "R_files/PF_GPS_sea_migration.RData")

#### ---STEP 4: temporal filter for autocorrelation #####

load("R_files/PF_GPS_sea_migration.RData") #called sea_data_migr
...
save(sf,file = "R_files/PF_sea_mgr_15_1hr.RData")

#### ---STEP 4: produce alternative points in time #####

#open data
load("R_files/PF_sea_mgr_15_1hr.RData") #called sf. with original times
...
save(alt_cmpl,file = "R_files/PF_temp_sp_filtered_15km_alt_14days.RData") #with 5 minutes added to 00:00

#### ---STEP 6: annotate alternative points with delta T #####

load("R_files/PF_temp_sp_filtered_15km_alt_14days.RData") #called alt_cmpl

#prep for track annotation on movebank
alt_cmpl_mb <- alt_cmpl %>%
  mutate(timestamp = paste(as.character(timestamp),"000",sep = "."),
         species = "PF") %>% 
  dplyr::select(-eobs.start.timestamp)

#rename columns
colnames(alt_cmpl_mb)[c(8,9)] <- c("location-long","location-lat")

write.csv(alt_cmpl_mb,"R_files/PF_temp_sp_filtered_15km_alt_14days.csv") #with 5 minutes added to 00:00
...
load("R_files/PF_temp_sp_filtered_15km_alt_ann_14days.RData") #called ann
...
mapview(x=PF,xcol="location.long",ycol="location.lat")
